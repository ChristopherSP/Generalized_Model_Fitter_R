#################################################################################################
#                               Modelo 5.0 - Valor Predito
#################################################################################################
mod_m5 = function(dt,indep.var,method = c('gbm','rf','svm','ensemble'),wrapper = 'h2o.glm.wrapper',family = 'gaussian',ntrees = 1000, max_depth = 15, learn_rate = 0.001,kernel = 'polynomial',degree=3,remove_tail = T,conf = 0.95,remove_zeros = T){
  
  outputColumnName = variables$outputColumnNames$SentenceValue
  dependentColumn = variables$independentVariables$SentenceValue
  
  method = match.arg(method)
  
  # Remove improcedente label
  prep = dt[get(variables$independentVariables$Sentence)==variables$probabilityClass$Sentence]
  ...
  switch(method,
         gbm = {
           model = h2o.gbm(y=y.dep, x=x.indep, training_frame = train.h2o, ntrees = ntrees, max_depth = max_depth, learn_rate = learn_rate, seed = 112358, model_id = paste0('mod5_',method))
         },
         rf = {
           mtry = floor(sqrt(length(x.indep)))
           model = h2o.randomForest(y=y.dep, x=x.indep, training_frame = train.h2o, ntrees = ntrees, max_depth = max_depth, mtries = mtry, seed = 112358, model_id = paste0('mod5_',method))
         },
         svm = {
  ...
  if(method!='svm'){
    ...
    save(model,file= paste0("./output/models/mod5_",method,'.RData'))
  }
