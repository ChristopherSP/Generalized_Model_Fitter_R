#################################################################################################
#                               Modelo 6.0 - Valor Predito Acordo
#################################################################################################
mod_m6 = function(dt,indep.var,method = c('gbm','rf','svm','ensemble'),wrapper = 'h2o.glm.wrapper',family = 'gaussian',ntrees = 1000, max_depth = 15, learn_rate = 0.001,kernel = 'polynomial',degree=3,remove_tail = T,conf = 0.95,remove_zeros = T){
  
  outputColumnName = variables$outputColumnNames$AgreementValue
  dependentColumn = variables$independentVariables$AgreementValue
  
  method = match.arg(method)
  
  # Remove improcedente label
  prep = dt[get(variables$independentVariables$AgreementPropensity)==variables$probabilityClass$AgreementPropensity]
  
  if(remove_zeros){
    prep = prep[get(dependentColumn) !=0]
  }
  if(remove_tail){
    # Remove 5% most extreme values
    prep = prep[get(dependentColumn) <= quantile(get(dependentColumn),conf, na.rm=T)]
  }
  ...
  localH2O = h2o.init(ip = "localhost", port = 54325,nthreads = -1,min_mem_size = '10M',max_mem_size = paste0(floor(as.integer(system("cat /proc/meminfo | head -n 1 | cut -d' ' -f9",intern = T))/(1024^2)) - 1,'G'))
  ...
  switch(method,
         gbm = {
           model = h2o.gbm(y=y.dep, x=x.indep, training_frame = train.h2o, ntrees = ntrees, max_depth = max_depth, learn_rate = learn_rate, seed = 112358, model_id = paste0('mod6_',method))
         },
         rf = {
           mtry = floor(sqrt(length(x.indep)))
           model = h2o.randomForest(y=y.dep, x=x.indep, training_frame = train.h2o, ntrees = ntrees, max_depth = max_depth, mtries = mtry, seed = 112358, model_id = paste0('mod6_',method))
         },
         svm = {
  ...
  if(method!='svm'){
    ...
    real_tg_test = (real_tg_test*scale_info_enc$scales[which(scale_info_enc$name == dependentColumn)]) + scale_info_enc$centers[which(scale_info_enc$name == dependentColumn)]
    real_tg_test = cbind(real_tg_test,test[,.(label)][complete.cases(test[,.SD,.SDcols = indep.var])])
    real_tg_train = cbind(train[,.(get(dependentColumn))][complete.cases(train[,.SD,.SDcols = indep.var])],predict.train)
    real_tg_train = (real_tg_train*scale_info_enc$scales[which(scale_info_enc$name == dependentColumn)]) + scale_info_enc$centers[which(scale_info_enc$name == dependentColumn)]
    real_tg_train = cbind(real_tg_train,train[,.(label)][complete.cases(train[,.SD,.SDcols = indep.var])])
  }
  ...
  if(method!='svm'){
    ...
    save(model,file= paste0("./output/models/mod6_",method,'.RData'))
  }
